...
func (u *Usecase) Disburse(ctx context.Context, req *dto.DisburseRequest) helper.JSONResult {
	...
	if !dto.DisburseAllowedMIMEs[mimeType] {
	...
	savePath := fmt.Sprintf("./uploads/%s", fmt.Sprintf("%d_%s_%s", req.ID, "disburse", req.Evidence.Filename))
	...
	loan.DisbursedAt = &req.DateOfDisbursement
	loan.DisbursedBy = employee.ID
	loan.Status = "disbursed"
	loan.DisbursalDocument = helper.ToPointer(strings.Replace(savePath, "./", "", 1))
