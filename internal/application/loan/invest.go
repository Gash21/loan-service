...
func (u *Usecase) Invest(ctx context.Context, req *dto.InvestRequest) helper.JSONResult {
	...
	if loanData.TotalInvested+req.Amount > loanData.TotalAmount {
	...
	if loanData.TotalAmount == loanData.TotalInvested {
	...
	savePath := fmt.Sprintf("./uploads/%s_%d_%d_%s.pdf", timestamp, *loanData.ID, *req.InvestorID, "agreement_letter")
